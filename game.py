...
def title(msg,x,y,w,h,ic,ac,action=None):
    ...
    smallText = pygame.font.Font('assets/fonts/AvenirMedium.ttf', 36)# pygame.font.SysFont("comicsansms",40)
    textSurf, textRect = text_objects(msg, smallText)
    textRect.center = ( (x+(w/2)), (y+(h/2)) )
    screen.blit(textSurf, textRect)
def dis(done):
    
    bg_surface = pygame.image.load('assets/ib.jpg').convert()
    bg_surface = pygame.transform.scale(bg_surface,(WIN_WIDTH,WIN_HEIGHT))
    screen.blit(bg_surface,(0,0))
    title("L E T T E R  R A I N",190,200,400,150,BLUE,BLUE)
    #button("PLAY!",150,450,100,50,green,bright_green)#enter the play function)
    #button("Quit",550,450,100,50,red,bright_red,quitgame)
    #button("View Leader Board",275,600,200,50,block_color,bright_green)#enter the leader board function) 
    play_btn_surface = pygame.image.load('assets/s.png').convert_alpha()
    play_btn_surface = pygame.transform.scale(play_btn_surface,(100,100))
    play_btn_box = play_btn_surface.get_rect(center = ((WIN_WIDTH/2 ,WIN_HEIGHT/2-10)))
    screen.blit(play_btn_surface,play_btn_box)
    quit_btn_surface = pygame.image.load('assets/q.png').convert_alpha()
    quit_btn_surface = pygame.transform.scale(quit_btn_surface,(100,100))
    quit_btn_box = play_btn_surface.get_rect(center = ((WIN_WIDTH/2 ,WIN_HEIGHT/2+200)))
    screen.blit(quit_btn_surface,quit_btn_box)
    leader_btn_surface = pygame.image.load('assets/leader.png').convert_alpha()
    leader_btn_surface = pygame.transform.scale(leader_btn_surface,(100,100))
    leader_btn_box = play_btn_surface.get_rect(center = ((WIN_WIDTH/2 ,WIN_HEIGHT/2+400)))
    screen.blit(leader_btn_surface,leader_btn_box)
    
    #button("LeaderBoard",200,500,100,200,black,black)
    for event in pygame.event.get():
            if event.type == pygame.QUIT:
                pygame.quit()
                quit()
            if event.type == pygame.MOUSEBUTTONUP:
                    if(play_btn_box.collidepoint(pygame.mouse.get_pos())):
                        startGame()
                    if(quit_btn_box.collidepoint(pygame.mouse.get_pos())):
                        pygame.quit()
                        quit()
                    if(leader_btn_box.collidepoint(pygame.mouse.get_pos())):
                        return 1                    		                        
...
while not done:
    ...
    for event in pygame.event.get():   # User did something
        if event.type==pygame.KEYDOWN:
            if event.key==pygame.K_SPACE:  # If user clicked close
